...
async function findPRForBranch(owner, repo, branch, token, proxyFetch) {
  const response = await proxyFetch(
    `https://api.github.com/repos/${owner}/${repo}/pulls?head=${encodeURIComponent(owner)}:${encodeURIComponent(branch)}&state=open`,
    {
      headers: {
...
function isBot(username) {
  ...
  return (
    normalizedUsername.endsWith("[bot]") ||
    KNOWN_BOT_LOGINS.has(normalizedUsername) ||
    normalizedUsername.endsWith("-bot") ||
    normalizedUsername.endsWith("_bot")
  );
}
