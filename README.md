...
6. Pushes `preview.png`, `preview.svg`, and `demo.gif` to a dedicated orphan branch (`ci-preview-artifacts`) under the path `pr-<number>/<run_id>/`, so the assets get permanent `raw.githubusercontent.com` URLs that GitHub renders inline in comments.
...
The `ci-preview-artifacts` branch is **auto-maintained by CI** and detached from `master` history (orphan branch). It is safe to delete or force-push; CI will recreate it on the next preview run. Exclude it from any stale-branch cleanup.
